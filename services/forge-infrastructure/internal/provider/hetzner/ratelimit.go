...
type Limiter struct {
	...
	lastDelay   time.Duration // for tests
}
...
// ObserveHeaders updates remaining/reset from RateLimit-* response headers.
func (l *Limiter) ObserveHeaders(h http.Header) {
	...
	if v := h.Get("RateLimit-Reset"); v != "" {
		if sec, err := strconv.ParseInt(v, 10, 64); err == nil {
			// Hetzner sends unix epoch seconds for reset.
			l.resetAt = time.Unix(sec, 0)
		}
	}
}

// Backoff429 computes the next delay for a 429, updates consecutive count, and sleeps.
// Respects RateLimit-Reset when present on the response headers.
func (l *Limiter) Backoff429(ctx context.Context, h http.Header) error {
	...
	if h != nil {
		if v := h.Get("RateLimit-Reset"); v != "" {
			if sec, err := strconv.ParseInt(v, 10, 64); err == nil {
				until := time.Unix(sec, 0).Sub(l.now())
				if until > delay {
					delay = until
				}
			}
...
// Consecutive429 returns how many consecutive 429s have been observed (tests).
func (l *Limiter) Consecutive429() int {
	l.mu.Lock()
	defer l.mu.Unlock()
