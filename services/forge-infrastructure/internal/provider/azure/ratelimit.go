...
// Acquire waits until an in-flight slot is available.
...
// ObserveHeaders updates remaining/reset from Retry-After / x-ms-ratelimit-* headers.
func (l *Limiter) ObserveHeaders(h http.Header) {
	...
	l.mu.Lock()
	defer l.mu.Unlock()
	if v := h.Get("x-ms-ratelimit-remaining-resource"); v != "" {
...
// Backoff429 computes the next delay for a 429 and sleeps.
