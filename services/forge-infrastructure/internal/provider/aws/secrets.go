...
// HTTPSecrets resolves AWS credentials JSON from Forge Secrets :access.
...
// EnvFallbackSecrets tries MapSecrets, then FORGE_INFRA_AWS_CREDENTIALS_JSON.
...
func (e *EnvFallbackSecrets) ResolveSecret(ctx context.Context, secretName string) (string, error) {
	...
	if v := strings.TrimSpace(os.Getenv("FORGE_INFRA_AWS_CREDENTIALS_JSON")); v != "" {
		return v, nil
	}
