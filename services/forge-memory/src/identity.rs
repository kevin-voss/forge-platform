...
impl Principal {
    ...
    /// Effective role for a project (token role, else membership role).
    pub fn role_for_project(&self, project_id: &str) -> Option<String> {
        ...
        if let Some(m) = &self.memberships {
            for p in &m.projects {
                if p.project_id.as_deref() == Some(project_id) {
...
/// Fixed principal for any non-empty token (tests / local doubles).
#[derive(Clone)]
pub struct FixedIdentityClient {
    pub principal: Principal,
}

#[async_trait::async_trait]
impl IdentityClient for FixedIdentityClient {
    async fn introspect(&self, token: &str) -> Result<Principal, String> {
