...
// DB wraps a pgx pool for the network schema.
type DB struct {
	Pool   *pgxpool.Pool
	Schema string
}
...
func (db *DB) Migrate(ctx context.Context) error {
	...
	if _, err := db.Pool.Exec(ctx, "CREATE SCHEMA IF NOT EXISTS network"); err != nil {
		return fmt.Errorf("create schema: %w", err)
	}
	if _, err := db.Pool.Exec(ctx, `
CREATE TABLE IF NOT EXISTS network.schema_migrations (
  ...
  applied_at TIMESTAMPTZ NOT NULL DEFAULT now()
	...
	for _, name := range names {
		...
		if err := db.Pool.QueryRow(ctx,
			`SELECT EXISTS (SELECT 1 FROM network.schema_migrations WHERE filename = $1)`, name,
		).Scan(&exists); err != nil {
		...
		if _, err := tx.Exec(ctx,
			`INSERT INTO network.schema_migrations (filename) VALUES ($1)`, name,
		); err != nil {
			_ = tx.Rollback(ctx)
