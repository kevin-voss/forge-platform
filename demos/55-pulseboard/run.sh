#!/usr/bin/env bash
# Demo 55: PulseBoard product scaffold + baseline deploy (epic 55.01).
# Usage:
#   demos/55-pulseboard/run.sh          # build → apply → Ready on board./api.pulseboard.localhost
...
ensure_platform() {
  ...
  if [[ "${need_recreate}" -eq 1 ]]; then
    echo "Recreating Control/Runtime/Gateway with demo 55 overlay..."
    "${COMPOSE[@]}" up -d --force-recreate \
      "${CONTROL_SERVICE}" "${RUNTIME_SERVICE}" "${GATEWAY_SERVICE}"
  else
    echo "Control/Gateway already configured for demo 55; ensuring they are up..."
