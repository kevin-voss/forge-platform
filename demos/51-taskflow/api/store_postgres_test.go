...
// Repository-layer CRUD against a real Postgres test container (or TASKFLOW_TEST_DATABASE_URL).
func TestTaskStorePostgresCRUD(t *testing.T) {
	dsn := strings.TrimSpace(os.Getenv("TASKFLOW_TEST_DATABASE_URL"))
	var cleanup func()
	if dsn == "" {
		var err error
	...
	created, err := store.CreateTask(ctx, "Buy milk")
	if err != nil {
	...
	if err != nil || got == nil || got.Title != "Buy milk" {
		t.Fatalf("get: got=%+v err=%v", got, err)
	}

	done := true
	patched, err := store.PatchTask(ctx, created.ID, nil, &done)
	if err != nil || patched == nil || !patched.Done {
	...
	gone, err := store.GetTask(ctx, created.ID)
	if err != nil || gone != nil {
...
func startPostgresContainer(t *testing.T) (string, func(), error) {
	...
	name := fmt.Sprintf("taskflow-pg-test-%d", time.Now().UnixNano())
	run := exec.Command(
		"docker", "run", "-d", "--rm",
		"--name", name,
	...
	dsn := fmt.Sprintf("postgres://test:test@127.0.0.1:%s/taskflow?sslmode=disable", hostPort)
